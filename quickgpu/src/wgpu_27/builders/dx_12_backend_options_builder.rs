#![allow(clippy::all)]

// The code in this file is generated by a script. Do not edit it directly.
//

pub use super::super::Nested;
pub use std::{borrow::Cow, num::NonZeroU32, ops::Range};
#[allow(unused_imports)]
use wgpu_27::util::DeviceExt;
pub trait Field {}
pub trait IsOptional {}
#[doc = "\nBuilder for [`wgpu_27::Dx12BackendOptions`]\n        \nSet all required fields and any optional fields, then call `build()`.\n\nBuilder field setters:\n  - [shader_compiler](Dx12BackendOptionsBuilder::shader_compiler) Optional, defaults to [wgpu_27::Dx12Compiler::Fxc]\n  - [presentation_system](Dx12BackendOptionsBuilder::presentation_system) Optional, defaults to [wgpu_27::wgt::Dx12SwapchainKind::DxgiFromHwnd]\n  - [latency_waitable_object](Dx12BackendOptionsBuilder::latency_waitable_object) Optional, defaults to [wgpu_27::wgt::Dx12UseFrameLatencyWaitableObject::Wait]\n"]
pub struct Dx12BackendOptionsBuilder<CS: State> {
    shader_compiler: CS::ShaderCompiler,
    presentation_system: CS::PresentationSystem,
    latency_waitable_object: CS::LatencyWaitableObject,
}
impl Dx12BackendOptionsBuilder<Empty> {
    pub fn new() -> Dx12BackendOptionsBuilder<Empty> {
        Dx12BackendOptionsBuilder {
            shader_compiler: ShaderCompilerEmpty,
            presentation_system: PresentationSystemEmpty,
            latency_waitable_object: LatencyWaitableObjectEmpty,
        }
    }
}
#[doc = "\nReturns [Dx12BackendOptionsBuilder] for building [`wgpu_27::Dx12BackendOptions`]\n        \nSet all required fields and any optional fields, then call `build()`.\n\nBuilder field setters:\n  - [shader_compiler](Dx12BackendOptionsBuilder::shader_compiler) Optional, defaults to [wgpu_27::Dx12Compiler::Fxc]\n  - [presentation_system](Dx12BackendOptionsBuilder::presentation_system) Optional, defaults to [wgpu_27::wgt::Dx12SwapchainKind::DxgiFromHwnd]\n  - [latency_waitable_object](Dx12BackendOptionsBuilder::latency_waitable_object) Optional, defaults to [wgpu_27::wgt::Dx12UseFrameLatencyWaitableObject::Wait]\n"]
pub fn dx_12_backend_options() -> Dx12BackendOptionsBuilder<Empty> {
    Dx12BackendOptionsBuilder::new()
}
pub struct ShaderCompilerEmpty;
impl Field for ShaderCompilerEmpty {}
pub trait ShaderCompilerIsEmpty {}
impl ShaderCompilerIsEmpty for ShaderCompilerEmpty {}
pub trait IsSetShaderCompiler {
    fn get(self) -> wgpu_27::Dx12Compiler;
}
impl IsSetShaderCompiler for ShaderCompilerEmpty {
    fn get(self) -> wgpu_27::Dx12Compiler {
        wgpu_27::Dx12Compiler::Fxc
    }
}
pub struct ShaderCompilerValue(pub wgpu_27::Dx12Compiler);
impl Field for ShaderCompilerValue {}
impl IsSetShaderCompiler for ShaderCompilerValue {
    fn get(self) -> wgpu_27::Dx12Compiler {
        self.0
    }
}
pub struct PresentationSystemEmpty;
impl Field for PresentationSystemEmpty {}
pub trait PresentationSystemIsEmpty {}
impl PresentationSystemIsEmpty for PresentationSystemEmpty {}
pub trait IsSetPresentationSystem {
    fn get(self) -> wgpu_27::wgt::Dx12SwapchainKind;
}
impl IsSetPresentationSystem for PresentationSystemEmpty {
    fn get(self) -> wgpu_27::wgt::Dx12SwapchainKind {
        wgpu_27::wgt::Dx12SwapchainKind::DxgiFromHwnd
    }
}
pub struct PresentationSystemValue(pub wgpu_27::wgt::Dx12SwapchainKind);
impl Field for PresentationSystemValue {}
impl IsSetPresentationSystem for PresentationSystemValue {
    fn get(self) -> wgpu_27::wgt::Dx12SwapchainKind {
        self.0
    }
}
pub struct LatencyWaitableObjectEmpty;
impl Field for LatencyWaitableObjectEmpty {}
pub trait LatencyWaitableObjectIsEmpty {}
impl LatencyWaitableObjectIsEmpty for LatencyWaitableObjectEmpty {}
pub trait IsSetLatencyWaitableObject {
    fn get(self) -> wgpu_27::wgt::Dx12UseFrameLatencyWaitableObject;
}
impl IsSetLatencyWaitableObject for LatencyWaitableObjectEmpty {
    fn get(self) -> wgpu_27::wgt::Dx12UseFrameLatencyWaitableObject {
        wgpu_27::wgt::Dx12UseFrameLatencyWaitableObject::Wait
    }
}
pub struct LatencyWaitableObjectValue(pub wgpu_27::wgt::Dx12UseFrameLatencyWaitableObject);
impl Field for LatencyWaitableObjectValue {}
impl IsSetLatencyWaitableObject for LatencyWaitableObjectValue {
    fn get(self) -> wgpu_27::wgt::Dx12UseFrameLatencyWaitableObject {
        self.0
    }
}
pub trait State {
    type ShaderCompiler: Field;
    type PresentationSystem: Field;
    type LatencyWaitableObject: Field;
}
pub struct Empty;
impl State for Empty {
    type ShaderCompiler = ShaderCompilerEmpty;
    type PresentationSystem = PresentationSystemEmpty;
    type LatencyWaitableObject = LatencyWaitableObjectEmpty;
}
pub struct SetShaderCompiler<CS>(CS);
impl<CS: State> State for SetShaderCompiler<CS> {
    type ShaderCompiler = ShaderCompilerValue;
    type PresentationSystem = CS::PresentationSystem;
    type LatencyWaitableObject = CS::LatencyWaitableObject;
}
pub struct SetPresentationSystem<CS>(CS);
impl<CS: State> State for SetPresentationSystem<CS> {
    type ShaderCompiler = CS::ShaderCompiler;
    type PresentationSystem = PresentationSystemValue;
    type LatencyWaitableObject = CS::LatencyWaitableObject;
}
pub struct SetLatencyWaitableObject<CS>(CS);
impl<CS: State> State for SetLatencyWaitableObject<CS> {
    type ShaderCompiler = CS::ShaderCompiler;
    type PresentationSystem = CS::PresentationSystem;
    type LatencyWaitableObject = LatencyWaitableObjectValue;
}
impl<CS: State> Dx12BackendOptionsBuilder<CS> {
    #[doc = "Setter for [wgpu_27::Dx12BackendOptions::shader_compiler]. Optional, defaults to [wgpu_27::Dx12Compiler::Fxc].\n"]
    pub fn shader_compiler(
        self,
        shader_compiler: wgpu_27::Dx12Compiler,
    ) -> Dx12BackendOptionsBuilder<SetShaderCompiler<CS>>
    where
        CS::ShaderCompiler: ShaderCompilerIsEmpty,
    {
        Dx12BackendOptionsBuilder {
            shader_compiler: ShaderCompilerValue(shader_compiler),
            presentation_system: self.presentation_system,
            latency_waitable_object: self.latency_waitable_object,
        }
    }
    #[doc = "Setter for [wgpu_27::Dx12BackendOptions::presentation_system]. Optional, defaults to [wgpu_27::wgt::Dx12SwapchainKind::DxgiFromHwnd].\n"]
    pub fn presentation_system(
        self,
        presentation_system: wgpu_27::wgt::Dx12SwapchainKind,
    ) -> Dx12BackendOptionsBuilder<SetPresentationSystem<CS>>
    where
        CS::PresentationSystem: PresentationSystemIsEmpty,
    {
        Dx12BackendOptionsBuilder {
            shader_compiler: self.shader_compiler,
            presentation_system: PresentationSystemValue(presentation_system),
            latency_waitable_object: self.latency_waitable_object,
        }
    }
    #[doc = "Setter for [wgpu_27::Dx12BackendOptions::latency_waitable_object]. Optional, defaults to [wgpu_27::wgt::Dx12UseFrameLatencyWaitableObject::Wait].\n"]
    pub fn latency_waitable_object(
        self,
        latency_waitable_object: wgpu_27::wgt::Dx12UseFrameLatencyWaitableObject,
    ) -> Dx12BackendOptionsBuilder<SetLatencyWaitableObject<CS>>
    where
        CS::LatencyWaitableObject: LatencyWaitableObjectIsEmpty,
    {
        Dx12BackendOptionsBuilder {
            shader_compiler: self.shader_compiler,
            presentation_system: self.presentation_system,
            latency_waitable_object: LatencyWaitableObjectValue(latency_waitable_object),
        }
    }
}
pub trait Complete:
    State<
    ShaderCompiler: IsSetShaderCompiler,
    PresentationSystem: IsSetPresentationSystem,
    LatencyWaitableObject: IsSetLatencyWaitableObject,
>
{
}
impl<
        CS: State<
            ShaderCompiler: IsSetShaderCompiler,
            PresentationSystem: IsSetPresentationSystem,
            LatencyWaitableObject: IsSetLatencyWaitableObject,
        >,
    > Complete for CS
{
}
impl<CS: Complete> Dx12BackendOptionsBuilder<CS> {
    pub fn build(self) -> wgpu_27::Dx12BackendOptions {
        wgpu_27::Dx12BackendOptions {
            shader_compiler: IsSetShaderCompiler::get(self.shader_compiler),
            presentation_system: IsSetPresentationSystem::get(self.presentation_system),
            latency_waitable_object: IsSetLatencyWaitableObject::get(self.latency_waitable_object),
        }
    }
}
impl Nested<wgpu_27::Dx12BackendOptions> for wgpu_27::Dx12BackendOptions {
    fn unnest(self) -> wgpu_27::Dx12BackendOptions {
        self
    }
}
impl<CS: Complete> Nested<wgpu_27::Dx12BackendOptions> for Dx12BackendOptionsBuilder<CS> {
    fn unnest(self) -> wgpu_27::Dx12BackendOptions {
        self.build()
    }
}
#[cfg(test)]
mod tests {
    #[allow(unused_imports)]
    #[allow(unused_imports)]
    use std::num::NonZeroU32;
    #[test]
    pub fn test_default() {
        assert_eq!(
            format!("{:#?}", super::dx_12_backend_options().build()),
            format!("{:#?}", wgpu_27::Dx12BackendOptions::default()),
        );
    }
}
