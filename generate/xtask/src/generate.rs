...
use crate::{
    ...
    utils::{generate_path, libs_path, rustfmt, without_args},
};
...
pub fn generate(version: Version) -> anyhow::Result<()> {
    ...
    let wgpu = {
        parse_crate(
            &mut analysis,
            generate_path("expanded/wgpu_types.rs"),
            generate_path(format!("{wgpu_source}/wgpu-types")),
            "wgpu_types",
            vec![],
        )
        .unwrap();
        ...
        parse_crate(
            &mut analysis,
            generate_path("expanded/wgpu_core.rs"),
            generate_path(format!("{wgpu_source}/wgpu-core")),
            "wgpu_core",
        ...
        parse_crate(
            &mut analysis,
            generate_path("expanded/wgpu_hal.rs"),
            generate_path(format!("{wgpu_source}/wgpu-hal")),
            "wgpu_hal",
        ...
        parse_crate(
            &mut analysis,
            generate_path("expanded/naga.rs"),
            generate_path(format!("{wgpu_source}/naga")),
            "wgpu_hal",
            vec![],
        )
        .unwrap();
        ...
        parse_crate(
            &mut analysis,
            generate_path("expanded/wgpu.rs"),
            generate_path(format!("{wgpu_source}/wgpu")),
            "wgpu",
            vec![],
        )
        .unwrap()
    ...
    let intro_path = libs_path(format!("{crate_name}/INTRO.md"));
    std::fs::write(intro_path.clone(), intro(version))?;

    let cargo_path = libs_path(format!("{crate_name}/Cargo.toml"));
    std::fs::write(cargo_path.clone(), cargo_toml(version))?;

    binder(version)?;

    let builders_path = libs_path(format!("{crate_name}/src/builders/"));
    let sh = Shell::new()?;
...
{code}
        ...
        let output_path =
            libs_path(format!("{crate_name}/src/builders/")).join(format!("{}.rs", name));
...
"
    ...
    let output_path = libs_path(format!("{crate_name}/src/builders/mod.rs"));
...
"#
    ...
    let output_path = libs_path(format!("{crate_name}/src/lib.rs"));
